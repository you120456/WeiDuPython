...
print("IT国内月报,开始运行：", datetime.datetime.now())
# =============================================================================
# 数据导入
# =============================================================================
path = r"C:\自动化数据存放\IT\月报" + "\\"  # 运行sql脚本数据存储路径
out_path = r"C:\自动化数据存放\IT\月报" + "\\"  # 数据输出路径
Collection_organization = pd.read_excel(path + "催员归属.xlsx")  # 导入催员归属
Mail_accept = pd.read_excel(path + "月小手机接收明细.xlsx")  # 导入小手机短信接收
Mail_send = pd.read_excel(path + "月小手机发送明细.xlsx")  # 导入小手机短信发送
Tcall_history = pd.read_excel(path + "月小手机通话明细.xlsx")  # 导入小手机通话明细
Group_online = pd.read_excel(path + "月小组上线天数.xlsx")  # 导入各小组本月上线总天数
area_online = pd.read_excel(path + "月区域上线天数.xlsx")  # 导入各区域本月上线总天数
Collection_online = pd.read_excel(path + "月催员上线天数.xlsx")  # 导入各催员本月上线总天数
call_history = pd.read_excel(path + "月国内通话详单.xlsx")  # 导入国内通话详单
call_batch = pd.read_excel(path + "月国内批呼.xlsx")  # 导入国内批呼
call_batch_summary = pd.read_excel(path + "月国内批呼汇总.xlsx")  # 导入国内批呼
...
def Tcall_summary(df1, df2):
    ...
    try:
        ...
        temp_df["平均每天外呼次数（本月）"] = round(temp_df["phone_num 外呼次数"] / temp_df["本月上线总数"], 0)
        temp_df["平均每天接通次数（本月）"] = round(temp_df["answer_num 接通次数"] / temp_df["本月上线总数"], 0)
        del temp_df["本月上线总数"]
    except:
        temp_df = temp_df
...
def call_detail(df1, df2):
    ...
    try:
        temp_df['平均每天外呼次数（本月）'] = round(temp_df["累计外呼次数"] / temp_df["上线天数"], 0)
        temp_df['平均每天接通次数（本月）'] = round(temp_df["累计接通次数(含IVR)"] / temp_df["上线天数"], 0)
        temp_df['平均每天通话时长(S)'] = round(temp_df["本月累计通话时长"] * 3600 / temp_df["上线天数"], 4)
        temp_df['平均每天振铃时长(S)'] = round(temp_df["本月累计振铃时长"] * 3600 / temp_df["上线天数"], 4)
        temp_df['平均每天总通话时长(S)'] = round(temp_df["本月总通话时长"] * 3600 / temp_df["上线天数"], 4)
        temp_df['平均每天通话时长(H)'] = round(temp_df["本月累计通话时长"] / temp_df["上线天数"], 4)
        temp_df['平均每天振铃时长(H)'] = round(temp_df["本月累计振铃时长"] / temp_df["上线天数"], 4)
        temp_df = temp_df[temp_df['平均每天总通话时长(S)'].notnull()]
        del temp_df["上线天数"]
    except:
        temp_df = temp_df
    return temp_df
...
def call_summary(df1, df2, df3, df4):
    ...
    temp_df['平均每天通话时长(H)'] = round(temp_df["本月累计通话时长"] / temp_df["上线天数"], 4)
    temp_df['平均每天振铃时长(H)'] = round(temp_df["本月累计振铃时长"] / temp_df["上线天数"], 4)
    temp_df['平均每通电话通话时长(s)'] = round(temp_df["本月累计通话时长"] * 3600 / temp_df["累计接通次数(含IVR)"], 4)
    temp_df['平均每通电话振铃时长(s)'] = round(temp_df["本月累计振铃时长"] * 3600 / temp_df["累计外呼次数"], 4)
    del temp_df["上线天数"]
    return temp_df
...
receiver_lst = ['yeyuhao@weidu.ac.cn', 'zhangjinyou@weidu.ac.cn', 'lijingyi@weidu.ac.cn',
                'fangyuanyuan@weidu.ac.cn', 'liwangchun@weidu.ac.cn', 'weilujie@weidu.ac.cn', 'panwei@weidu.ac.cn']
