...
time_end = now
# time_start = now - datetime.timedelta(days=10)
# time_end = now - datetime.timedelta(days=3)



# time_start = now.replace(year=2024, month=5, day=10)  # 数据开始日期
# time_end = now.replace(year=2024, month=5, day=17, hour=0, minute=0, second=0, microsecond=0)  # 数据截止日期+1day
...
sql1 = '''
        ...
        FROM
                ...
                JOIN ods_audit_call_history_extend ce ON ch.id = ce.source_id 
        WHERE
                ch.call_at >= '{0}'
                and ch.call_at < '{1}'
                AND ch.call_channel = 1 
         '''.format(str(time_start), str(time_end))
finish_df = config.chinese_bd_engine_read(sql1)
...
# 邮件主题
    mail_title = 'IT分小时过程数据'
    sender_email = email_variable
    sender_password = email_password
    recipient_email = to_email
    # attachment_path1 = r".\印尼排名.xlsx"
    attachment_path1 = r"E:/自动化数据存放/IT/IT分小时过程数据.xlsx"
    # 邮件正文
    mail_content = """
    <!DOCTYPE html>
    <html>
    ...
    attachment1.add_header("Content-Disposition", "attachment", filename=("gbk", "", "IT分小时过程数据.xlsx"))
    # 添加附件到邮件
...
mail_list = ["liwangchun@weidu.ac.cn"]
# mail_list = ["a838831723@gmail.com"]
for i in mail_list:
