...
# time_start = now.replace(year=2024, month=1, day=1) # 开始日期
# time_end = now.replace(year=2024, month=3, day=8) # 数据截止日期
...
sql_uo_indonesia = '''
                SELECT
                    ...
                    d.online_day 
                FROM
                    ods_fox_user_organization uo
                    LEFT JOIN ( SELECT DISTINCT NAME, NO AS TL_NO FROM ods_fox_sys_user ) AS cc ON uo.parent_user_name = cc.
                    NAME LEFT JOIN ( SELECT DISTINCT id, NO AS co_NO FROM ods_fox_sys_user ) AS b ON uo.user_id = b.id
                    LEFT JOIN ( SELECT DISTINCT user_id, dimission_date 
                    FROM ods_fox_sys_user_extend ) AS c ON uo.user_id = c.user_id
                    LEFT JOIN ( SELECT user_id, min( online_day ) online_day 
                    FROM ods_fox_online_days WHERE online_day_flag = 1 GROUP BY user_id ) AS d ON uo.user_id = d.user_id
    '''
uo_indonesia_df = config.indonesia_bd_engine_read(sql_uo_indonesia)
...
sql_input_indonesia = '''
                ...
                FROM
                    ...
                    JOIN ods_fox_asset a ON cr.asset_id = a.asset_id
                    JOIN ods_fox_debtor_asset da ON a.asset_id = da.asset_id 
                WHERE
                    cr.`batch_num` IS NULL 
                    AND cr.`late_days` = 1 
                    AND cr.`create_at` >= '{}'
...
input_indonesia_df = config.indonesia_bd_engine_read(sql_input_indonesia)
...
sql_order_indonesia = '''
                    ...
                    END AS 状态,
                    content as "内容"
                    FROM ods_fox_work_order wo 
                    where wo.create_at >=  '{}'
                    and wo.create_at <  '{}'
    '''.format(str(month_start), str(time_end))

# order_indonesia_df = config.indonesia_bd_engine_read(sql_order_indonesia)
...
sql_call_indonesia = '''
            ...
            FROM
                ...
                LEFT JOIN ods_audit_call_history_extend se ON ch.id = se.source_id 
            WHERE
                ch.`call_at` >=  '{}'
                and ch.`call_at`<'{}'
                AND ch.call_channel = 1 -- 呼出
                '''.format(str(time_start), str(time_end))
call_indonesia_df = config.indonesia_bd_engine_read(sql_call_indonesia)
...
call_indonesia_tmk_sql = '''SELECT
                        ...
                        FROM
                            ...
                            LEFT JOIN ods_fox_tel_sale_contact_record tcr ON tch.call_uuid = tcr.call_uuid 
                        WHERE
                            tch.dial_time >= '{0}' 
                            and tch.dial_time <'{1}' 
                               '''.format(str(time_start), str(time_end))
call_indonesia_tmk_df = config.indonesia_bd_engine_read(call_indonesia_tmk_sql)
...
tel_indonesia_wa_sql = '''SELECT 
                ...
                WHERE
                        ...
                        AND t.title LIKE '%TEL_SALE_WHATSAPP_LINK%'
                               '''.format(str(month_start), str(time_end))
tel_indonesia_wa_df = config.indonesia_bd_engine_read(tel_indonesia_wa_sql)
...
indonesia_wa_sql = '''SELECT 
                    ...
                    debtor_id 债务人ID ,
                    enc_phone as 手机号密文 
                    FROM ods_fox_whatsapp_session_log t,
                         ods_fox_sys_user su 
                                    WHERE
                    t.dunner_id = su.id 
                     and status =1
                                            ...
                                            AND t.create_date < '{1}' 
                               '''.format(str(month_start), str(time_end))
# indonesia_wa_df = config.indonesia_bd_engine_read(indonesia_wa_sql)
...
receiver_lst = ['yeyuhao@weidu.ac.cn', 'huxiaoqing@weidu.ac.cn', 'yuzheng@weidu.ac.cn', 'lichi@weidu.ac.cn','huangtianci@weidu.ac.cn',
                'zhangjinyou@weidu.ac.cn', 'lijingyi@weidu.ac.cn', 'xiejinhui@weidu.ac.cn','lihui2@weidu.ac.cn','bailijuan@weidu.ac.cn','liuyuanjin@weidu.ac.cn']
...
receiver_lst = ['yeyuhao@weidu.ac.cn', 'huxiaoqing@weidu.ac.cn', 'yuzheng@weidu.ac.cn', 'lichi@weidu.ac.cn',
                'zhangjinyou@weidu.ac.cn', 'lijingyi@weidu.ac.cn', 'xiejinhui@weidu.ac.cn', 'lihao2@weidu.ac.cn',
                'chenyu2@weidu.ac.cn','lihui2@weidu.ac.cn','bailijuan@weidu.ac.cn','liuyuanjin@weidu.ac.cn','huangtianci@weidu.ac.cn']
