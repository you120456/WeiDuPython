...
day_uo_df = config.indonesia_bd_engine_read(sql_day_uo, database='fox_dw')  # 每日架构查询
...
user_info = config.indonesia_bd_engine_read(sql_user_info)  # 上线天数
...
collect_recovery_df = config.indonesia_bd_engine_read(sql_collect_recovery)  # 实收记录
...
sql_new_misson_log = '''  
                        ...
                        AND ((assign.assign_asset_late_days = 1 AND assign.mission_group_name LIKE "A1%") 
                        OR (assign.assign_asset_late_days in (1,4) AND assign.mission_group_name LIKE "A2%" ) 
                            OR ( assign.assign_asset_late_days = 1 AND assign.mission_group_name LIKE "A Mix%" ) 
                            OR ( assign.assign_asset_late_days in (1,8) AND assign.mission_group_name LIKE "B1%") 
                                 OR ( assign.assign_asset_late_days in (1,15) AND assign.mission_group_name LIKE "B2%") 
                                 OR ( assign.assign_asset_late_days in (1,31) AND assign.mission_group_name LIKE "M%"))
                        ) a
                        left join 
                        (
                            SELECT 
                        ...
                        WHERE DATE(cd.work_day)=DATE(a.分案时间)
                        and cd.user_id = a.被分配到催员ID
                        AND cd.attendance_status_flag in (4,6,5,7)
                        )
                        -- 剔除撤案原因不在统计范围内的案件
                        and NOT (a.撤案原因 in ('GROUP_EXCHANGE_WITHDRAWAL_CASE','RESUME_CASE_DISMISSAL','UNEVEN_WITHDRAW_CASE','SETTLED_BEFORE_DIVISION','WITHDRAW_D_2_CASES_WHEN_THERE_ARE_OVERDUE_CASES') and a.撤案原因 IS NOT NULL and a.撤案时间 IS NOT NULL )
                        and NOT (a.撤案原因 in ('FREEZE_DEBTOR','MODERATE_WITHDRAW_CASE','FREEZE') and a.撤案时间 IS NOT NULL and a.撤案原因 IS NOT NULL and rpa.`催回金额` IS  NULL )
                        and NOT (a.撤案原因 in ('RISK_REPORTING_TRANSFERRED_TO_CUSTOMER_SERVICE','AUDIT_ABNORMAL_WITHDRAW_THE_CASE','AUDIT_COMPLAINT_TEND_WITHDRAWAL_CASE') and a.撤案时间 IS NOT NULL 
                        and a.撤案原因 IS NOT NULL and rpa.`催回金额` IS  NULL and date(a.撤案时间) = date(a.`分案时间`))
                        group by a.`被分配到催员ID`,date(a.`分案时间`)
                        UNION all
                        select 
                        a.`被分配到催员ID` as '催员ID',
                        DATE(a.`分案时间`) as '日期',
                        ...
                        and assign.mission_log_create_at >= '{0}'
                        and assign.mission_log_create_at < '{1}' 
                        AND ((assign.assign_asset_late_days = -2 AND assign.mission_group_name = "PreRemind-D-2" ) 
                            OR (assign.assign_asset_late_days = 0 AND assign.mission_group_name = "PreRemind-D0" ) )
                        ) a
                        left join 
                        (
                            SELECT 
...
new_misson_log_df = config.indonesia_bd_engine_read(sql_new_misson_log)  # 催员本周分案(新案)
...
new_D1_misson_log_df = config.indonesia_bd_engine_read(sql_new_D1_misson_log)  # 新案D1催回率
...
sql_misson_log = '''  
                        ...
                        and assign.mission_log_create_at >= '{0}'
                        and assign.mission_log_create_at < '{1}' 
                        AND ((assign.mission_group_name LIKE "A1%") 
                        OR (assign.mission_group_name LIKE "A2%" ) 
                            OR (assign.mission_group_name LIKE "A Mix%" ) 
                            OR ( assign.mission_group_name LIKE "B1%") 
                                 OR (assign.mission_group_name LIKE "B2%") 
                                 OR (assign.mission_group_name LIKE "M%"))
                        ) a
                        left join 
                        (
                            SELECT 
                        ...
                        and assign.mission_log_create_at >= '{0}'
                        and assign.mission_log_create_at < '{1}' 
                        AND ((assign.mission_group_name = "PreRemind-D-2" ) 
                            OR (assign.mission_group_name = "PreRemind-D0" ) )
                        ) a
                        left join 
                        (
                            SELECT 
...
misson_log_df = config.indonesia_bd_engine_read(sql_misson_log)  # 催员本周分案(所有案)


# 字段3日均分案
def Columns3(misson_log_df, user_info, finish_df):
    ...
    temp_df_week = temp_df_week.groupby(by=['组别', '是否新人', '主管'],
                                        as_index=False).agg({"户数": "sum", "分案本金": "sum", "上线天数": "sum"})
    sum_temp_df_week = temp_df_week.groupby(by=['组别', '是否新人'], as_index=False).agg(
        {"户数": "sum", "分案本金": "sum", "上线天数": "sum"})
    sum_temp_df_week['主管'] = '汇总'
    temp_df_week = pd.concat([temp_df_week, sum_temp_df_week])
    temp_df_week['日人均分案个数'] = round(temp_df_week['户数'] / temp_df_week['上线天数'], 0)
    temp_df_week['日人均分案本金'] = round(temp_df_week['分案本金'] / temp_df_week['上线天数'], 0)
...
#                     group by 1,2
#                     '''.format(str(time_start), str(time_end))
# InHands_Mssion_df = config.local_database(sql_InHands_Mssion, base_name='indonesia_fox')  # 计算在手
#
#
...
call_df = config.indonesia_bd_engine_read(sql_call)  # 通话时间
...
sql_mission_call = '''
                          select            
                            ...
                            group by 催员ID,债务人ID,债务人关系,拨打号码) call_data
                                                                                           JOIN (select
                                                            ...
                                                            -- 剔除撤案原因不在统计范围内的案件
                                                            and NOT (mlur.mission_log_unassign_reason in ('GROUP_EXCHANGE_WITHDRAWAL_CASE','RESUME_CASE_DISMISSAL','UNEVEN_WITHDRAW_CASE','SETTLED_BEFORE_DIVISION'
                                                                                ,'WITHDRAW_D_2_CASES_WHEN_THERE_ARE_OVERDUE_CASES','FREEZE_DEBTOR','MODERATE_WITHDRAW_CASE','FREEZE') 
                                                                                and mlur.mission_log_unassign_reason IS NOT NULL and unassign.mission_log_create_at IS NOT NULL )
                                                            and NOT (mlur.mission_log_unassign_reason in ('RISK_REPORTING_TRANSFERRED_TO_CUSTOMER_SERVICE','AUDIT_ABNORMAL_WITHDRAW_THE_CASE','AUDIT_COMPLAINT_TEND_WITHDRAWAL_CASE')
                                                                     and unassign.mission_log_create_at IS NOT NULL 
                                                                    and mlur.mission_log_unassign_reason IS NOT NULL  and date(unassign.mission_log_create_at) = date(assign.mission_log_create_at))
                                                            GROUP BY 1,2)	mission 
                                                                                ON call_data.`债务人ID` = mission.债务人ID
                                                                                AND call_data.`催员ID` = mission.催员ID
                    '''.format(str(time_start), str(time_end))

mission_call_df = config.indonesia_bd_engine_read(sql_mission_call)  # 周期内分案通话记录
...
mission_debtor_df = config.indonesia_bd_engine_read(sql_mission_debtor)  # 周期内分案通话记录
...
def Columns6(mission_call_df, user_info, finish_df):
    user_info = user_info.drop_duplicates(subset=['催员ID'], keep='first')
    temp_df = pd.merge(mission_call_df, user_info, on=['催员ID'], how='left')
...
def Columns7(mission_call_df, user_info, finish_df, mission_debtor_df):
    user_info = user_info.drop_duplicates(subset=['催员ID'], keep='first')
    temp_call_df = pd.merge(mission_call_df, user_info, on=['催员ID'], how='left')
...
def Columns8(mission_call_df, user_info, finish_df, mission_debtor_df):
    ...
    # 总接通债务人
    temp_total_call_df = pd.merge(mission_call_df, user_info, on=['催员ID'], how='left')
...
finish_df = finish_df[(~finish_df['组别'].str.contains('OS')) | (finish_df['主管'] != '汇总')]
...
out_path = r"D:\DailyReport\印尼报表\电催战报\周报" + "\\"
