...
def 国内分案回款(a, b, c, first_day, yesterday, 新案):
    ...
    FROM
    ods_fox_mission_log ml
    LEFT JOIN ods_fox_mission_log_unassign_reason mlur ON ml.mission_log_id = mlur.assign_mission_log_id and mlur.create_at >= "{3}"
    LEFT JOIN ods_fox_mission_log unml ON mlur.mission_log_id = unml.mission_log_id and unml.mission_log_create_at >="{3}"
    AND unml.mission_log_operator = 'unassign' 
    AND ( unml.mission_log_create_at >= "{3}" AND unml.mission_log_create_at <= "{4}" ) 
    LEFT JOIN ods_fox_collect_attendance_dtl cad ON ml.assigned_sys_user_id = cad.user_id and cad.create_at >="{3}"
    AND DATE(ml.mission_log_create_at) = cad.work_day
    LEFT JOIN ods_fox_collect_recovery cr ON ml.mission_log_id = cr.mission_log_id and cr.assigned_date >= "{3}"
    AND ( cr.create_at >= "{3}" AND cr.create_at <= "{4}" ) 
    AND cr.batch_num IS NOT NULL
    LEFT JOIN ods_fox_asset_period_recovery apr ON cr.batch_num = apr.batch_num and apr.create_at >= "{3}"
    LEFT JOIN ods_fox_collect_attendance_dtl cadcr ON cr.sys_user_id = cadcr.user_id
    AND DATE(cr.create_at) = cadcr.work_day
    LEFT JOIN ods_fox_asset asset ON ml.mission_log_asset_id = asset.asset_id and asset.asset_create_at >="{3}"
    WHERE
    ml.mission_log_operator = 'assign'
    AND (ml.mission_log_create_at >= "{3}" AND ml.mission_log_create_at <= "{4}")
    AND ml.mission_group_name NOT IN ('PHCC','IVR','WIZ-IVR','cultivate')) detail
