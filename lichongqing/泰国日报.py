...
def 数据(link,新案):
    ...
    data= 业务组(link)
    df11 = pd.read_sql(sql.添加业务组(first_day), link)
    df1 = pd.read_sql(sql.架构表1(first_day,yesterday,",".join(data["组别"].apply(lambda x: f'"{x}"'))),link)
    ...
    df6 = pd.read_sql(sql.外呼(first_day,yesterday+" 23:59:59"),link)
    df7 = pd.read_sql(sql.晋升时间(),link)
    df8 = pd.read_sql(sql.usertable(),link)
    df9 = pd.read_sql(sql.总分案回款(first_day,yesterday+" 23:59:59"),link)
    ...
    df14 = pd.read_sql(sql.案件量监控2(first_day, yesterday), link)  #####20241105 新增 案件量监控 fei国内
    分案监控 = pd.read_sql(sql.分案监控2(third_day, yesterday), link)  #####20241105
    承载监控 = pd.read_sql(sql.承载监控2(third_day, yesterday), link)  #####20241105
    团队过程指标 = pd.read_sql(sql2.团队过程指标3(first_day, yesterday), link)
    团队过程指标IVR = pd.read_sql(sql2.团队过程指标IVR_泰国(first_day, yesterday), link)
    组长过程指标 = pd.read_sql(sql2.组长过程指标3(third_day, yesterday), link)
    组员过程指标 = pd.read_sql(sql2.组员过程指标3(third_day, yesterday), link)
...
if __name__ == '__main__':
    ...
    # for i in range(1,31):
    #     yesterday = "2024-10-{:02d}".format(i)
    ...
    新案1 = ['因不可抗力因素温和催收,正常撤案,冻结债务人,冻结',
             ...
             """(ml.mission_group_name LIKE 'P%' AND ml.assign_asset_late_days = -2)
    ...
    save("{0}{1}".format(yesterday,current_file_name), path, ce=ce, tl=tl, sp=sp,resign=resign,top催回率=top催回率, 日均分案=日均分案)
