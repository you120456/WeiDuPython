...
def 数据(link,新案):
    ...
    团队过程指标IVR = pd.read_sql(sql2.团队过程指标IVR_菲律宾(first_day, yesterday), link)
    组长过程指标 = pd.read_sql(sql2.组长过程指标1(third_day, yesterday), link)
    组员过程指标 = pd.read_sql(sql2.组员过程指标1(third_day, yesterday), link)
...
if __name__ == '__main__':
    ...
    新案2 = ['因不可抗力因素温和催收,正常撤案,冻结债务人,冻结',
           ...
           """(ml.mission_group_name LIKE 'P%' AND ml.assign_asset_late_days = -2)
                   ...
                   OR (ml.mission_group_name LIKE 'A-2%' AND ml.assign_asset_late_days = 1)
    ...
    案件量监控=df14,分案监控=分案监控,承载监控=承载监控
         ,团队过程指标=团队过程指标,团队过程指标IVR=团队过程指标IVR,组长过程指标=组长过程指标,组员过程指标=组员过程指标,完全出催数据=完全出催数据,完全出催数据_上个月=完全出催数据_上个月,完全出催数据_上上个月=完全出催数据_上上个月)
    #美化文件
