...
def 数据(link,新案):
    global data,df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13,df14,df15,团队过程指标,组长过程指标,组员过程指标,分案监控,承载监控,完全出催数据,完全出催数据中国_上个月,完全出催数据中国_上上个月,团队过程指标IVR
    data= 业务组(link)
    df1 = pd.read_sql(sql.中国架构表(first_day,yesterday,",".join(data["组别"].apply(lambda x: f'"{x}"'))),link)
    ...
    df9 = pd.read_sql(sql.国内总分案回款(first_day,yesterday),link)
    ...
    分案监控 = pd.read_sql(sql.分案监控2(third_day, yesterday), link)  #####20241105
    承载监控 = pd.read_sql(sql.承载监控2(third_day, yesterday), link)  #####20241105
    团队过程指标 = pd.read_sql(sql2.团队过程指标1(first_day, yesterday), link)
    团队过程指标IVR = pd.read_sql(sql2.团队过程指标IVR_中国(first_day, yesterday), link)
    组长过程指标 = pd.read_sql(sql2.组长过程指标中国(third_day, yesterday), link)
    组员过程指标 = pd.read_sql(sql2.组员过程指标中国(third_day, yesterday), link)
    完全出催数据 = pd.read_sql(sql2.完全出催数据中国(first_day, yesterday), link)#完全出催数据中国
    完全出催数据中国_上个月 = pd.read_sql(sql2.完全出催数据中国_上个月(first_day, yesterday), link)  # 完全出催数据中国
    完全出催数据中国_上上个月 = pd.read_sql(sql2.完全出催数据中国_上上个月(first_day, yesterday), link)  # 完全出催数据中国
...
if __name__ == '__main__':
    ...
    # for i in range(1,12):
    #     yesterday = f"2024-11-{i:02}".format(i)
    ...
    新案1 = ['因不可抗力因素温和催收,正常撤案,冻结债务人,冻结,超期案件协商还款',
          ...
          """(ml.mission_group_name LIKE 'P%' AND ml.assign_asset_late_days = 0)
          OR (ml.mission_group_name LIKE 'A-1%' AND ml.assign_asset_late_days = 1)
          OR (ml.mission_group_name LIKE 'A-2%' AND ml.assign_asset_late_days IN (1,4))
          OR (ml.mission_group_name LIKE 'B1-2%' AND ml.assign_asset_late_days IN (1,8))
          OR (ml.mission_group_name LIKE 'B1-1%' AND ml.assign_asset_late_days IN (1,8))
          OR (ml.mission_group_name LIKE 'M2-TJ%' AND ml.assign_asset_late_days IN (1,31,61))
          OR (ml.mission_group_name LIKE 'M2-YT%' AND ml.assign_asset_late_days IN (1,31,61))
          OR (ml.mission_group_name LIKE 'B2%' AND ml.assign_asset_late_days IN (1,16))"""]
    #获取数据,修改数仓,新案
    数据(sc.ch(),新案1)
    #运行过程
    ...
    ce.columns = ["业务组","组别","远程/现场","委外/自营","帐龄","新案","正式主管","主管工号","主管","组长工号","组长","催员工号","催员",
    ...
    save("{0}{1}".format(yesterday,current_file_name), path, ce=ce, tl=tl, sp=sp,resign=resign,临时表=临时表,top催回率=top催回率,日均分案=日均分案)
    ...
    案件量监控=df15,分案监控=分案监控,承载监控=承载监控,团队过程指标=团队过程指标,团队过程指标IVR=团队过程指标IVR,组长过程指标=组长过程指标,组员过程指标=组员过程指标,完全出催数据=完全出催数据,完全出催数据中国_上个月=完全出催数据中国_上个月,完全出催数据中国_上上个月=完全出催数据中国_上上个月)
    #美化文件
    ...
    # to_email1 = ["zhoumengjie@weidu.ac.cn"]
