...
def 过程():
    ...
    #个人每日架构汇总
    gb2=mg1.groupby(["asset_group_name","manager_user_no","manager_user_name","leader_user_no","leader_user_name","user_no","组员","user_id"],as_index= False).agg(
        新案分案本金 = ("新案分案本金",'sum'),
        新案回款本金 =("新案回款本金","sum"),
        新案展期费用=("新案展期费用","sum"),
        总实收=("总实收","sum"),
...
if __name__ == '__main__':
    ...
    ce,tl,sp=gc1(pa,dfs,first_day,rs)
    # print(rs)
    top催回率 = n1.top催回率(ce)
    # #保存文件
    current_file_name = os.path.basename(__file__).split(".")[0]
    path = "./{}".format(current_file_name)
    save("{0}{1}".format(yesterday,current_file_name), path, ce=ce, tl=tl, sp=sp,top催回率= top催回率)
    #美化文件
