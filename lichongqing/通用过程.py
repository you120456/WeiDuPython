...
def gc1(pa, dfs,dfm,first_day,yesterday,rs,country):
    if country.startswith("墨"):
        from 排名区间 import mxg实收区间 as 实收range
        from 排名区间 import mxg催回率 as 催回率range
        from 排名区间 import calculate_range1 as 综合排名range
    elif country.startswith("泰"):
        from 排名区间 import flb区间 as 实收range
        from 排名区间 import flb区间 as 催回率range
        # from 排名区间 import tg实收区间 as 实收range
        # from 排名区间 import tg催回率 as 催回率range
        from 排名区间 import calculate_range1 as 综合排名range
    elif country.startswith("巴"):
        from 排名区间 import flb区间 as 实收range
        from 排名区间 import flb区间 as 催回率range
        from 排名区间 import calculate_range1 as 综合排名range
    elif country.startswith("菲"):
        from 排名区间 import flb区间 as 实收range
        from 排名区间 import flb区间 as 催回率range
        from 排名区间 import calculate_range1 as 综合排名range
    elif country.startswith("中"):
        from 排名区间 import flb区间 as 实收range
        from 排名区间 import flb区间 as 催回率range
        from 排名区间 import calculate_range1 as 综合排名range
    pa["Repaid principal adjust"] = pa["新案回款本金"] + pa["新案展期费用"]
    pa["Individual Collection Rate"] = np.where(
        pa["新案分案本金"] != 0,
        (pa["新案回款本金"] + pa["新案展期费用"]) / pa["新案分案本金"],
        0  # 当分母为0时返回0
    )
    pa["Newly enrolled Yes/ No"] = (pd.to_datetime(yesterday) - pd.to_datetime(pa["min_date"])).dt.days.apply(lambda x: "YES" if x < 30 else "NO")
    ...
    try:
        pa["Ranking intervals"] = pa.apply((lambda x: 实收range(x, pa, "Gross collectiion ranking")), axis=1)
        pa["Integrated Ranking interval"] = pa.apply((lambda x: 综合排名range(x, pa)), axis=1)
        pa["参与排名业务组人数"]=pa[pa["总天数"]>0].groupby(["asset_group_name"],as_index= False)["user_id"].transform('count')
        pa["日均实收排名区间"] = pa.apply((lambda x: 实收range(x, pa,"日均实收排名")), axis=1)
        pa["催回率排名区间"] = pa.apply((lambda x: 催回率range(x, pa, "催回率排名")), axis=1)
    except Exception as e:
        pa["Ranking intervals"] = None
        pa["Integrated Ranking interval"] = None
        pa["参与排名业务组人数"] = None
        pa["日均实收排名区间"] = None
        pa["催回率排名区间"] = None
    ...
    try:
        pa_filtered["Gross collectiion ranking"] = pa_filtered.groupby(["asset_group_name"])["总实收"].rank(ascending=False,
          method="first")
        pa_filtered["Ranking intervals"] = pa_filtered.apply((lambda x: 实收range(x, pa_filtered, "Gross collectiion ranking")),
          axis=1)
        pa_filtered["催回率排名"] = pa_filtered.groupby("asset_group_name")["Individual Collection Rate"].rank(ascending=False,
          method="first")
        pa_filtered["催回率排名区间"] = pa_filtered.apply((lambda x: 催回率range(x, pa, "催回率排名")), axis=1)
        pa_filtered["日均实收排名"] = pa_filtered.groupby("asset_group_name")["日均实收"].rank(ascending=False, method="first")
        pa_filtered["日均实收排名区间"] = pa_filtered.apply((lambda x: 实收range(x, pa,"日均实收排名")), axis=1)
        ...
        pa_filtered["Integrated Ranking interval"] = pa_filtered.apply((lambda x: 综合排名range(x, pa_filtered)), axis=1)
        pa_filtered["参与排名业务组人数"] = pa_filtered.groupby(["asset_group_name"], as_index=False)["user_id"].transform('count')
        ...
        try:
            ...
            del e
    ce = pa[[
     "业务组","asset_group_name","office_name","Out_self","账龄","新案","正式主管","正式主管中文名","manager_user_no","manager_user_name","leader_user_no","leader_user_name","user_no",
     ...
     "Integrated Ranking interval","参与排名业务组人数"
        ,"分案本金总","催回本金总","实收总"
    ]]
    ce.columns = [
         "业务组","Group","office_name","Out_self","账龄","新案","正式主管","正式主管中文名","Supervisor_no","Supervisor","leader_user_no","Team Leader","user_no",
         ...
         "Integrated Ranking","Integrated Ranking interval","参与排名业务组人数"
            , "分案本金总", "催回本金总", "实收总"]
    ...
    g1 = dfs.groupby(["asset_group_name","office_name","Out_self","账龄","manager_user_no","manager_user_name","leader_user_no","leader_user_name"], as_index=False)[
        ('新案分案本金','Repaid principal adjust','总实收','总天数','分案本金总','催回本金总','实收总')].sum()
    g2 = dfr.groupby(["asset_group_name","manager_user_no","manager_user_name","leader_user_no","leader_user_name"], as_index=False)[('新案分案本金','Repaid principal adjust','总实收','总天数')].sum()
    ...
    tl = g1[["asset_group_name","office_name","Out_self","账龄","manager_user_no","manager_user_name","leader_user_no","leader_user_name",
     "新案分案本金","Repaid principal adjust","Team Collection Rate1",
     "Rank1","总实收","总天数","Ave.Gross collection","Rank2"
        ,"分案本金总", "催回本金总", "实收总"
             ]].merge((g2[["asset_group_name","manager_user_no","manager_user_name",
    ...
    tl.columns = ["Group","office_name","Out_self","账龄","Supervisor_no","Supervisor","leader_user_no","Team Leader","Divided principal adjust",
    ...
    m1 = dfm.groupby(["asset_group_name","Out_self", "manager_user_no", "manager_user_name"], as_index=False)[
        ('新案分案本金','Repaid principal adjust','总天数'
         ,"分案本金总","催回本金总","实收总"
         )].sum()
    ...
    sp = m1[["asset_group_name","Out_self","manager_user_no", "manager_user_name", "Team Collection Rate1"
        ,"分案本金总","催回本金总","实收总"
             ]].merge(m2, on=[
     "asset_group_name", "manager_user_no", "manager_user_name"],
      how="left")
    sp = sp[["asset_group_name","Out_self","manager_user_no","manager_user_name","Team Collection Rate1",
     "Team Collection Rate2","新案分案本金","Repaid principal adjust","总天数","分案本金总", "催回本金总", "实收总"]]

    sp.columns = ["Group","Out_self","manager_user_no","Supervisor","Team Collection Rate1",
    "Team Collection Rate2 >7","Divided principal adjust >7","Repaid principal adjust >7","Online days >7","分案本金总","催回本金总","实收总"]
    return ce, tl, sp